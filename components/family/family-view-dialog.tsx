  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 999, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(26,16,8,0.35)', backdropFilter: 'blur(2px)' }}
      ...
      <div className="glass-warm shadow-cloud"
        style={{ width: '100%', maxWidth: '40rem', maxHeight: '90vh', overflowY: 'auto', padding: '1.5rem 2rem', borderTop: '2px solid rgba(15, 42, 31,0.4)' }}
        ...
        {record.photos && record.photos.length > 0 && (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '0.5rem', marginBottom: '1.25rem' }}>
            {record.photos.map((photo, i) => (
              <img key={i} src={photo} alt={`Photo ${i + 1}`} style={{ width: '100%', aspectRatio: '1', objectFit: 'cover', borderRadius: '4px', border: '1px solid rgba(15, 42, 31,0.15)' }} />
            ))}
          </div>
        )}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.25rem', marginBottom: '1.25rem' }}>
          <div>
            <p style={{ fontFamily: 'var(--font-inter)', fontSize: '0.62rem', letterSpacing: '0.24em', textTransform: 'uppercase', color: 'rgba(15, 42, 31,0.65)', marginBottom: '0.5rem' }}>Personal Details</p>
          ...
          <div>
            {getSpouses(record).length > 0 && (
              <>
                <p style={{ fontFamily: 'var(--font-inter)', fontSize: '0.62rem', letterSpacing: '0.24em', textTransform: 'uppercase', color: 'rgba(15, 42, 31,0.65)', marginBottom: '0.5rem' }}>
        ...
        {(record.address || record.cell_numbers.length > 0 || record.email) && (
          <div style={{ marginBottom: '1.25rem', paddingBottom: '1rem', borderBottom: '1px solid rgba(15, 42, 31,0.12)' }}>
            <p style={{ fontFamily: 'var(--font-inter)', fontSize: '0.62rem', letterSpacing: '0.24em', textTransform: 'uppercase', color: 'rgba(15, 42, 31,0.65)', marginBottom: '0.5rem' }}>Contact</p>
            <div style={{ fontFamily: 'var(--font-inter)', fontSize: '0.8rem', color: '#1A1008', lineHeight: 2 }}>
        ...
        <div>
          ...
          <div style={{ overflow: 'hidden', border: '1px solid rgba(15, 42, 31,0.12)' }}>
              ...
              <tbody>
                {record.children.length === 0 ? (
                  ...
                  record.children.map((child) => (
                    <tr key={child.code} style={{ borderBottom: '1px solid rgba(15, 42, 31,0.08)' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', paddingTop: '1.25rem' }}>
          <button style={{ fontFamily: 'var(--font-inter)', fontSize: '0.72rem', letterSpacing: '0.14em', textTransform: 'uppercase', padding: '0.55rem 1.4rem', border: '1px solid rgba(15, 42, 31,0.3)', background: 'transparent', color: 'rgba(26,16,8,0.45)', cursor: 'pointer', transition: 'all 0.2s' }}
            onClick={() => onOpenChange(false)}
